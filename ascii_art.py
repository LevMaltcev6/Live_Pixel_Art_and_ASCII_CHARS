...
class ArtConverter:
    def __init__(self, path='video/hh.mp4', font_size=15):
        pg.init()
        self.path = path
        self.capture = cv2.VideoCapture(0) # 0 for camera | "path" for video
        self.image = self.get_image()
        self.RES = self.WIDTH, self.HEIGHT = self.image.shape[0], self.image.shape[1]
        self.surface = pg.display.set_mode(self.RES)
        self.clock = pg.time.Clock()

        self.ASCII_CHARS = '.- 012?><:;-56789'
        ...
        self.rec_fps = 25
        self.record = False
        self.recorder = cv2.VideoWriter('output/asciiwd.mp4', cv2.VideoWriter_fourcc(*'mp4v'), self.rec_fps, self.RES)
    ...
    def get_image(self):
        ...
        # transposed_image = (self.cv2_image)
